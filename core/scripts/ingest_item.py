...
GIT_ROOT = PATHS.repo_root or ROOT
...
def load_topic_keyword_map() -> dict[str, str]:
    mapping_file = ROOT / "data" / "topic_keyword_map.json"
    if not mapping_file.exists():
        return TOPIC_BY_KEYWORD
    ...
    for key, value in data.items():
        if isinstance(key, str) and isinstance(value, str):
            clean[key] = value
    return clean or TOPIC_BY_KEYWORD
...
def propose_topics(keywords: list[str], valid_topic_order: list[str], topic_by_keyword: dict[str, str]) -> list[str]:
    ...
    if not topics:
        topics = {valid_topic_order[0]}
    ordered = [t for t in valid_topic_order if t in topics]
...
def cmd_prepare(args: argparse.Namespace) -> int:
    ...
    valid_topic_order = [entry["id"] for entry in topics_data if isinstance(entry.get("id"), str)]
    if not valid_topic_order:
        raise ValueError("topics.json must contain at least one topic id")
    topic_by_keyword = load_topic_keyword_map()
...
def cmd_ingest(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        print(f"- en_path: {PATHS.display_path(CONTENT_EN / slug / 'index.md')}")
        print(f"- zh_path: {PATHS.display_path(CONTENT_ZH / slug / 'index.md')}")
        return 0
    ...
    print(f"- EN: {PATHS.display_path(en_index)}")
    print(f"- ZH: {PATHS.display_path(zh_index)}")
    if attachments:
    ...
    if args.run_checks:
        ...
        validate_script = Path(__file__).resolve().with_name("validate_content.py")
        code, stdout, stderr = run_command([sys.executable, str(validate_script)], cwd=ROOT)
    ...
    if args.git_push:
        ...
        add_targets = [str(en_dir), str(zh_dir)]
        if KEYWORD_PROPOSALS_FILE.exists():
            add_targets.append(str(KEYWORD_PROPOSALS_FILE))
        print("Running git add...")
        code, _, stderr = run_command(["git", "add", *add_targets], cwd=GIT_ROOT)
        if code != 0:
            print(stderr, file=sys.stderr)
            return code

        print("Running git commit...")
        code, stdout, stderr = run_command(["git", "commit", "-m", commit_message], cwd=GIT_ROOT)
        sys.stdout.write(stdout)
        sys.stderr.write(stderr)
        if code != 0:
            return code
        ...
        code, stdout, stderr = run_command(["git", "push", "origin", "main"], cwd=GIT_ROOT)
        sys.stdout.write(stdout)
        sys.stderr.write(stderr)
        if code != 0:
            return code
